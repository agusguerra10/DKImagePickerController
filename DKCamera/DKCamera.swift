...
open class DKCamera: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    open func setupUI() {
        ...
        contentView.addGestureRecognizer(UIPinchGestureRecognizer(target: self, action: #selector(handleZoom(_:))))
        contentView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(handleFocus(_:))))
    }
            ...
            DispatchQueue.global().async(execute: {
                if let connection = stillImageOutput.connection(with: AVMediaType.video) {
                    ...
                    stillImageOutput.captureStillImageAsynchronously(from: connection, completionHandler: { (imageDataSampleBuffer, error) in
                        if error == nil, let data = imageDataSampleBuffer {
                            let imageData = AVCaptureStillImageOutput.jpegStillImageNSDataRepresentation(data)
